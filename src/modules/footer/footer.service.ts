  ...
  private toDto(footer: Footer, schedules: ScheduledProgram[]): FooterResponseDto {
    return {
      ...
      contact: {
        ...
        schedule: schedules.map(schedule => ({ [schedule.title]: `${schedule.scheduledDay} ${format(parse(schedule.startTime, "HH:mm:ss", new Date()), "h:mm a")} ${schedule.additionalTimes ? schedule.additionalTimes.split(",").map(it => `, ${format(parse(it, "HH:mm:ss", new Date()), "h:mm a")}`).join('') : ""}` }))
      },
